...
public enum Config {
    ...
    public ByteBuffer toBytes() {
        ...
        for (int i = 0; i < 2; i++) {
            result.put(intToBytes(delay[i]).array());
            result.put(intToBytes(lifeTime[i]).array());
            result.put(intToBytes(percentage[i]).array());
            result.put(intToBytes(priority[i]).array());
        }
        result.put(boolsToByte(new boolean[]{ai[0], ai[1], upperPanel, report}));
        return result;
    }
    public void fromBytes(ByteBuffer bytes) {
        for (int i = 0; i < 2; i++) {
            delay[i] = bytes.getInt(i*4*4);
            lifeTime[i] = bytes.getInt((i*4+1)*4);
            percentage[i] = bytes.getInt((i*4+2)*4);
            priority[i] = bytes.getInt((i*4+3)*4);
        }
